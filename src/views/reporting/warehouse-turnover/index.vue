<template>
  <div class="warehouse-turnover">
    <h2>{{ $t('menu.reporting.warehouseTurnover') }}</h2>
    
    <!-- Filter Section -->
    <div style="margin: 10px 0; padding: 12px; background: #f5f5f5; border: 1px solid #e6e6e6; border-radius: 4px;">
      <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
        <el-select v-model="filters.ownerId" :placeholder="$t('menu.reporting.owner') || 'Owner'" clearable style="width: 180px">
...
const groupedData = computed(() => {
  ...
  if (!filters.value.dictId) {
    ...
    dataToGroup.forEach(item => {
      ...
      const ownerKey = item.owner_name || 'Unknown'
      const groupKey = `${monthKey}_${ownerKey}`
  ...
  const groups = {}
  dataToGroup.forEach(item => {
    const key = item.owner_name || 'Unknown'
    if (!groups[key]) {
      groups[key] = []
    }
...
const handleSearch = async () => {
  ...
  try {
    const params = {
      keeperid: currentUser.value?.userId
    }
